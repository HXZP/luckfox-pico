#!/bin/bash
#
# install_rknn_env.sh - 一键安装 RKNN-Toolkit2 PC 转换环境
#
# 方案说明：
# 1. 本脚本用于上位机 Ubuntu/WSL 环境，不用于开发板端运行。
# 2. RKNN-Toolkit2 对 Python 版本有要求，当前使用独立 Conda 环境，避免污染系统 Python。
# 3. 环境安装在仓库根目录的 .rknn/miniconda3 下，Conda 环境名为 RKNN-Toolkit2。
# 4. 默认使用 Python 3.9、rknn-toolkit2 2.3.2 和 CPU 版 PyTorch。
# 5. 实测直接 clone GitHub rknn-toolkit2 仓库容易卡住，因此本脚本优先从 PyPI 安装稳定发布包。
#
# 使用说明：
