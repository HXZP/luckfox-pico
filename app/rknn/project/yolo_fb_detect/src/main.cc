...
int main(int argc, char **argv)
{
    ...
    int bench_frame_count;

    bench_frame_count = get_benchmark_frame_count();
    ...
    {
        printf("YOLO inference disabled, camera and framebuffer display only\n");
    }
    start_mjpeg_server();

    // 初始化 fb0 显示，后续直接把带检测框的 BGR 图像写到 RGB565 framebuffer。
    ...
    while (g_running)
    {
        ...
        cv::putText(bgr,text,cv::Point(0, 20),
                    cv::FONT_HERSHEY_SIMPLEX,0.5,
                    cv::Scalar(0,255,0),1);
        if ((frame_index % MJPEG_UPDATE_INTERVAL) == 0)
        {
